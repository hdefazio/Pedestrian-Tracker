...
CreatePedestrianTracker(const std::string& reid_model,
    ...
    if (!reid_model.empty() && !reid_weights.empty()) {
        //-----------------------------
        CnnConfig reid_config(reid_model, reid_weights, 0.5, 1920, 1200);
...
bool ParseAndCheckCommandLine(int argc, char *argv[]) {
    ...
    //---------------------
    //if(FLAGS_d_thr.empty()) {
        //throw std::logic_error("Paramete -d_thr is not set");
    //}
...
int main_work(int argc, char **argv) {
    ...
    //------------------------
    auto detector_threshold = FLAGS_d_thr;

    auto input_h = FLAGS_in_h;

    auto input_w = FLAGS_in_w;
    ...
    DetectorConfig detector_confid(det_model, det_weights, detector_threshold, input_h, input_w);
